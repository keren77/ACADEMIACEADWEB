<?php
	...
	function emailExiste($email)
	{
		...
		$stmt = $mysqli->prepare("SELECT id_usuario FROM tbl_usuarios WHERE correo = ? LIMIT 1");
	...
	function enviarEmail($email, $usuario, $asunto, $cuerpo){
		...
		$mail->Username = 'kyanes917@gmail.com'; //Modificar
		$mail->Password = 'Shinigami'; //Modificar

		$mail->setFrom('kyanes917@gmail.com', 'KY'); //Modificar
	...
	function generaTokenPass($userid)
	{
		global $mysqli;

		$token = generateToken();

		$stmt = $mysqli->prepare("UPDATE tbl_usuarios SET token_password=?, password_request=1 WHERE id = ?");
	...
	function getValor($campo, $campoWhere, $valor)
	{
		...
		$stmt = $mysqli->prepare("SELECT $campo FROM tbl_usuarios WHERE $campoWhere = ? LIMIT 1");
